...
class TKTexture: NSObject {
    ...
    //MARK: - Creation
    init(imageNamed: String, mipmaped:Bool){
        imageName = imageNamed
        path = Bundle.main.pathForImageResource(imageNamed)
        width    = 0
        height   = 0
        depth    = 1
    ...
    func loadTexture(view: TKView, flip: Bool) {
        let img = Bundle.main.image(forResource: self.imageName)!
        ...
        let texDescriptor = MTLTextureDescriptor.texture2DDescriptor(pixelFormat: pixelFormat, width: Int(width), height: Int(height), mipmapped: isMipmaped)
        self.target = texDescriptor.textureType
        self.texture = view.device!.makeTexture(descriptor: texDescriptor)
        ...
        //    if (isMipmaped == true){
        //        generateMipMapLayersUsingSystemFunc(texture: texture, device: view.device!, commandQ: view.renderer.commandQueue, block: { (buffer) -> Void in
    ...
    func image(mipLevel: Int) -> TKImage {
        ...
        let image = TKImage(cgImage: imgRef!, size: TKSize(width: mipmapedWidth, height: mipmapedHeight))
        return image
    }
